...
import type { AidrOpenAI } from '../../client.js';
import { AidrAIGuardBlockedError } from '../../core/error.js';
...
export class AidrResponses extends Responses {
  protected _client: AidrOpenAI;

  constructor(client: AidrOpenAI) {
    super(client);
    this._client = client;
  }
  ...
  override create(
    ...
    return this._client.aiGuardClient
      .guardChatCompletions({ guard_input: { messages }, event_type: 'input' })
      .then((inputGuardResponse) => {
        if (inputGuardResponse.status === 'Accepted') {
          return super.create(body, options);
        }
        ...
        if (
          ...
          inputGuardResponse.result?.guard_output?.messages
        ) {
          body.input = inputGuardResponse.result.guard_output.messages as {
            role: 'system' | 'user' | 'assistant' | 'developer';
            content: string;
          }[];
        }
